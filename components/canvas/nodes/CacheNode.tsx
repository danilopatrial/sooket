...
export type { CacheNodeData } from "@/lib/node-types";
import type { CacheNodeData } from "@/lib/node-types";

type Tops = { key: number; value: number; output: number; hit: number };

function CacheNodeComponent({ data, selected }: NodeProps) {
  const { zoom } = useViewport();
  const d         = data as unknown as CacheNodeData;
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-[#212124] border-b border-white/[0.06] rounded-t-2xl">
        ...
        <div className="min-w-0">
          <p className="text-[13px] font-semibold text-white leading-none">Cache</p>
      ...
      <div className="px-4 py-3 space-y-2.5 nodrag">
        ...
        <div ref={keyRowRef} className="flex items-center gap-2 h-6">
          <span className={cn(
            ...
            connected.includes("key") ? "text-teal-300" : "text-white/30"
          )}>
            key
          </span>
        ...
        <div ref={valueRowRef} className="flex items-center gap-2 h-6">
          <span className={cn(
            "text-[10px] uppercase tracking-wider font-semibold",
            connected.includes("value") ? "text-teal-300" : "text-white/30"
          )}>
            value
          </span>
